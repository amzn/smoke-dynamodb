...
public extension AWSDynamoDBCompositePrimaryKeyTable {
    ...
    func monomorphicBulkWriteWithFallback<AttributesType, ItemType>(_ entries: [WriteEntry<AttributesType, ItemType>]) -> EventLoopFuture<Void> {
        // fall back to singel operation if the write entry exceed the statement length limitation
        ...
        let futures = nonBulkWriteEntries.map { nonBulkWriteEntry -> EventLoopFuture<Void> in
        ...
        return EventLoopFuture.whenAllSucceed(futures, on: self.eventLoop).flatMap { _ in
            return self.monomorphicBulkWrite(bulkWriteEntries)
        }
    }
    ...
    func monomorphicBulkWriteWithFallback<AttributesType, ItemType>(_ entries: [WriteEntry<AttributesType, ItemType>]) async throws {
        ...
        return try await monomorphicBulkWrite(entries)
    }
